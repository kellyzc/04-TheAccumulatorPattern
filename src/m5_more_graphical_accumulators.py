...
def run_test_draw_circles_from_rectangle():
    ...
    rectangle = rg.Rectangle(rg.Point(400, 350), rg.Point(450, 370))
    rectangle.fill_color = "green"
    rectangle.outline_color = "red"
    draw_circles_from_rectangle(3, 7, rectangle, window1)

    rectangle = rg.Rectangle(rg.Point(360, 320), rg.Point(390, 340))
    ...
    rectangle = rg.Rectangle(rg.Point(480, 550), rg.Point(500, 600))
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for i in range(m):
        circle = rg.Circle(rg.Point(rectangle.get_center().x - rectangle.get_width() / 2 - rectangle.get_height() / 2 * (2 * i + 1), rectangle.get_center().y), rectangle.get_height() / 2)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)
    for k in range(n):
        circle = rg.Circle(rg.Point(rectangle.get_center().x, rectangle.get_center().y - rectangle.get_height() / 2 - rectangle.get_width() / 2 * (2 * k + 1)), rectangle.get_width() / 2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for i in range(n):
        line = rg.Line(rg.Point(rectangle1.get_center().x - rectangle1.get_width() / 2 * i, rectangle1.get_center().y + rectangle1.get_height() / 2 * i),
                       rg.Point(rectangle2.get_center().x - rectangle1.get_width() / 2 * i, rectangle2.get_center().y + rectangle1.get_height() / 2 * i))
        line.thickness = 5
        if i % 2 == 0:
